...
import com.wokun.tysl.mypersonmoney.adapter.CashAdapter;
import com.wokun.tysl.mypersonmoney.bean.CashBean;
import com.wokun.tysl.mypersonmoney.bean.CashResponse;
...
public class CashDetalisActivity extends BaseBindingActivity implements
    ...
    private List<CashBean> mDataList;
    private BaseQuickAdapter<CashBean, BaseViewHolder> mAdapter;
    ...
    private Request initRequest() {
        ...
        mRequest = OkGo.<BaseResponse<CashBean>>post(Constants.BASE_URL + Constants.UCENTER_MONEY_DETAIL_URL).tag(this);
    ...
    public BaseQuickAdapter<CashBean, BaseViewHolder> initAdapter() {
        mAdapter = new CashAdapter(R.layout.item_lastmoney,null);
    ...
    public void loadData(final boolean isRefresh) {
        ...
        mRequest.execute(new JsonCallback<BaseResponse<CashResponse>>(Constants.WITH_TOKEN,Constants.UCENTER_MONEY_DETAIL_URL) {
            @Override
            public void onSuccess(Response<BaseResponse<CashResponse>> response) {
                BaseResponse body = response.body();
                if(body == null)return;
                if(body.isState()){
                    mMultipleStatusView.showContent();
                    CashResponse mylastmoneyResponse = (CashResponse) body.getData();
                    if(mylastmoneyResponse==null){return;}
                    setData(isRefresh, mylastmoneyResponse.getMoneyList());
                }
            }
        });
    }
    ...
    public void setData(boolean isRefresh, List<CashBean> data) {
        mNextRequestPage++;
