...
//商品订单确认页面
public class GoodsOrderConfirmationActivity4 extends BaseBindingActivity {
    ...
    public void init() {
        ...
        goodsID =   getIntent().getStringExtra(Constants.GOODS_ID);
    ...
    private void loadData() {
     //   Log.e("订单", cart_id_str + "//%%" + getIntent().getDoubleExtra(Constants.ORDER_TOTAL, 0.00d));
        ...
        OkGo.<BaseResponse<OrderShowBean>>post(Constants.BASE_URL + Constants.GO_CART_ORDER_SHOW)
                .tag(this)
                .params(Constants.GOODS_ID, goodsID)
                .execute(new JsonCallback<BaseResponse<OrderShowBean>>(Constants.WITH_TOKEN, Constants.GO_CART_ORDER_SHOW) {
                    @Override
                    ...
                    Log.e("订单2",body.getMsg()+"");
                        if (body == null) return;
                        Log.e("支付宝进来了", "进来了进来了2"+response.getRawResponse());
                        if (body.isState()) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 99 && resultCode == 20) {
            ...
            actionCartGetFreightPrice(cartIdStr, province_id);
            setAddress(name, mobile, sAddress);
            selectAddress.setVisibility(View.GONE);
            hasAddress.setVisibility(View.VISIBLE);
        }
    ...
    private void doAlipay() {
        Log.e("支付宝支付参数","name;"+name+"mobile;"+mobile+"sAddress;"+sAddress+"remark;"+etRemark.getText().toString().trim()+"order_total;"+order_total+"freight_total_price;"+new_freight_total_price
        +"pay_price;"+pay_price+"cartIdStr;"+cartIdStr+"province_id;"+province_id);
        OkGo.<BaseResponse<Alipay>>post(Constants.BASE_URL + Constants.ORDER_SUBMIT_URL)
                ...
                .execute(new JsonCallback<BaseResponse<Alipay>>(Constants.WITH_TOKEN, Constants.ORDER_SUBMIT_URL) {
                    ...
                    @Override
                    public void onError(Response<BaseResponse<Alipay>> response) {
                        super.onError(response);
                        Log.e("订单6进来了", "进来了进来了1"+response.body());
                        Log.e("订单6进来了", "进来了进来了2"+response.getRawResponse());
                        Log.e("订单6进来了", "进来了进来了3"+response.message());
                    }
                });
    }
                ...
                .execute(new JsonCallback<BaseResponse<WXPayResult>>(Constants.WITH_TOKEN, Constants.ORDER_SUBMIT_URL) {
                    ...
                    public void onSuccess(Response<BaseResponse<WXPayResult>> response) {
                        ...
                        if (body.isState()) {
                            ...
                            Log.e("订单6进来了", "进来了进来了1"+data.getAppid());
                            //在服务端签名
                            WXPayUtils.WXPayBuilder builder = new WXPayUtils.WXPayBuilder();
                        //   WXPay wxPay = new WXPay.Builder()
                            builder.setAppId(data.getAppid())
                                    ...
                                    .build()
                                    .toWXPayNotSign(GoodsOrderConfirmationActivity4.this);
    ...
    protected void alipay(final String body) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                PayTask alipay = new PayTask(GoodsOrderConfirmationActivity4.this);
